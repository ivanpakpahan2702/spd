...
def create_scheduled_task():
    ...
    sql = f"INSERT INTO schedule_task (name, description, created_at, due_date, token) VALUES ('{name}','{description}', '{created_at}', '{due_date}','{token}')"
    print(sql)
    try:
        with get_db() as db:
            schedule_task = db.execute(sql).fetchall()
    ...
    if send_schedule_notif:
        get_all_user_email_sql = f"SELECT email FROM users WHERE role='pegawai'"
        try:
            with get_db() as db:
...
def edit_scheduled_task(token):
    ...
    due_date = json_data['due_date']
    send_schedule_notif = json_data['notif_variable']
    sql = f"UPDATE schedule_task SET name='{name}', description='{description}', created_at='{created_at}', due_date='{due_date}' WHERE token='{token}'"
    print(sql)
    try:
        with get_db() as db:
            schedule_task = db.execute(sql).fetchall()
