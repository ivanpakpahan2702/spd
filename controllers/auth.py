    ...
    if request.method == 'GET':
        ...
        try:
            with get_db() as db:
                ...
                db.commit()
            flash(f'Hello {name}, Registration successful. Please log in.', 'success')
    ...
    if request.method == 'POST':
        ...
        with get_db() as db:
            ...
            if user:
                ...
                db.execute(
                    'UPDATE users SET reset_token = ?, reset_token_time = ? WHERE id = ?',
                    (token, now, user['id'])
                )
                db.commit()
...
def reset_password(user_id, token):
    with get_db() as db:
        ...
        token_time_str = user['reset_token_time']
        token_time = datetime.strptime(token_time_str, '%Y-%m-%d %H:%M:%S.%f') if token_time_str else None
        now = datetime.now()

        if token_time and (now - token_time) > timedelta(seconds=3600):
